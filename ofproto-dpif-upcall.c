             ...
             struct hmap *misses)
{
    ...
    for (i = 0; i < FLOW_MISS_MAX_BATCH; i++) {
		...
		VLOG_INFO( "wpf_first：datapath port %d,flow->priority:%d,flow->dst_ip:%d,flow->source_ip:%d,flow->tp_src:%d,flow->tp_dst:%d,flow->proto:%d", 
		odp_in_port,flow.skb_priority,flow.nw_dst,flow.nw_src,flow.tp_src,flow.tp_dst,flow.nw_proto);
		VLOG_INFO( "wpf_second：flow->pkt_mark:%d,flow->recirc_id:%d,flow->dl_type:%d,flow->vlan_tci:%d,flow->nw_frag:%d,flow->nw_tos:%d,flow->nw_ttl:%d,flow->tcp_flags:%d,flow->pad:%d,flow->dp_hash:%d", 
		flow.pkt_mark,flow.recirc_id,flow.dl_type,flow.vlan_tci,flow.nw_frag,flow.nw_tos,flow.nw_ttl,flow.tcp_flags,flow.pad,flow.dp_hash);
