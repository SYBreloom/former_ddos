...
# 发包host与对应的指令字典
host_cmds = {
    #'h101': 'ping 192.168.5.122 -c 15',
    'h101': 'ping 192.168.4.118 -c 15',
    'h106': '',
    'h114': '',
	'h118': 'ifconfig'
...
def do_send_packet(host):
    print exp.get_node(host).cmd(host_cmds[host])
...
def get_topo():
	...
	os.popen('ovs-vsctl add-port s5 eth1')       #绑定s5和host的eth1接口
	os.popen('ifconfig s5 192.168.0.1 netmask 255.255.0.0')
    return topo
